...
public:
	/**
	* Bind a TCP socket object to a local endpoint given an address. If the address is empty, 0.0.0.0 is used
	*/
    ...
    /**
    * Bind a UDP socket object to a local endpoint given an address. If the address is empty, 0.0.0.0 is used
    */
    template <class SocketType>
    static void BindToLocalAddress(const std::string& address, asio::ip::udp::endpoint& endpoint, SocketType& socket, std::error_code& ec)
    {
        auto string = address.empty() ? "0.0.0.0" : address;
        auto addr = asio::ip::address::from_string(string, ec);
        if (!ec)
